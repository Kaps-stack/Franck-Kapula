...
// =========================================================
// FORMATAGE DES DATES (EXPERIENCE & EDUCATION)
// =========================================================
...
const formatPeriod = (startDate, endDate, isCurrent) => {
    ...
    if (isCurrent) {
        return start ? `${start} — Présent` : "Présent";
    }
...
<template>
    ...
    <div class="bg-[#08090d] text-white">
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <i
                                    class="fa-solid fa-hand-peace text-amber-100"
                                ></i>
                            </Motion>
                    ...
                    >
                        <div class="flex items-start">
                            ...
                            >
                                <i
                                    class="fa-solid fa-pencil text-amber-200"
                                ></i>
                            </Motion>
                        </div>
                    </Motion>
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                <Small> Télécharger mon CV </Small>
                            </a>
                        </Motion>
                    </div>
                </div>
            ...
            <div class="mx-auto max-w-7xl">
                ...
                >
                    ...
                    >
                        ...
                        <Body class="max-w-md text-slate-400 sm:text-right">
        ...
        <section
            id="experience"
            class="border-y border-white/10 bg-[#160027] px-6 py-20"
        >
            <div class="mx-auto max-w-7xl">
                <Motion
                    :initial="{
                    ...
                    :viewport="{ once: true }"
                >
                    <Title class="text-white"> 02 — Experience </Title>
                ...
                <div
                    ...
                    class="relative mt-16 ml-2 border-l border-white/10"
                >
                    <Motion
                    ...
                    >
                        <article class="relative pb-12 pl-8 sm:pl-12">
                            <!-- POINT NEON DESIGNE -->
                            <span
                                class="absolute -left-[5px] top-1.5 h-2.5 w-2.5 rounded-full bg-indigo-400 shadow-[0_0_8px_#818cf8]"
                            />

                            <div class="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
                                <Title size="md" class="text-white">
                                    {{ experience.position }}
                                </Title>

                                <!-- AFFICHAGE DE LA PERIODE & BADGE PRESENT -->
                                <div class="flex items-center gap-2 font-mono text-xs text-slate-400">
                                    <span>
                                        {{ formatPeriod(experience.start_date, experience.end_date, experience.current) }}
                                    </span>
                                    <span
                                        v-if="experience.current"
                                        class="rounded-full border border-indigo-400/30 bg-indigo-500/10 px-2.5 py-0.5 font-mono text-[10px] font-semibold text-indigo-300"
                                    >
                                        En poste
                                    </span>
                                </div>
                            ...
                            <div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-indigo-400">
                                <a
                                ...
                                <Small v-else class="text-indigo-400">
                                    {{ experience.company }}
                                </Small>

                                <span v-if="experience.location" class="text-slate-500">• {{ experience.location }}</span>
                                <span v-if="experience.type" class="rounded bg-white/5 px-2 py-0.5 font-mono text-[10px] text-slate-400">
                                    {{ experience.type }}
                                </span>
                            </div>

                            <div class="mt-4 max-w-3xl">
                                <Body class="text-slate-400 leading-relaxed">
        ...
        <section id="education" class="border-b border-white/10 bg-[#302f2f] px-6 py-20">
            <div class="mx-auto max-w-7xl">
                <Motion
                    :initial="{
                        opacity: 0,
                    ...
                    :viewport="{ once: true }"
                >
                    <Title class="text-white"> 03 — Education </Title>

                    <Heading class="mt-4 text-emerald-400"> Formation </Heading>
                </Motion>

                <div
                    v-if="education?.length"
                    class="mt-12 divide-y divide-white/10 border-y border-white/10"
                >
                    <Motion
                    ...
                    >
                        <article
                            class="grid gap-4 py-8 md:grid-cols-[80px_1fr_2fr]"
                        >
                            <Small class="font-mono text-xs text-slate-500">
                            ...
                            <div>
                                ...
                                <div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-emerald-400">
                                    <a
                                        v-if="item.institution_url"
                                    ...
                                    <Small v-else class="text-emerald-400">
                                        {{ item.institution }}
                                    </Small>

                                    <span v-if="item.field" class="text-slate-400">— {{ item.field }}</span>
                                    <span v-if="item.location" class="text-slate-500">• {{ item.location }}</span>
                                </div>

                                <!-- PERIODE ET BADGE EN COURS -->
                                <div class="mt-2 flex items-center gap-2 font-mono text-xs text-slate-400">
                                    <span>
                                        {{ formatPeriod(item.start_date, item.end_date, item.current) }}
                                    </span>
                                    <span
                                        v-if="item.current"
                                        class="rounded-full border border-emerald-400/30 bg-emerald-500/10 px-2 py-0.5 font-mono text-[10px] font-semibold text-emerald-300"
                                    >
                                        En cours
                                    </span>
                                </div>
